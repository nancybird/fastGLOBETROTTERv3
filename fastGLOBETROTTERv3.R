...
## then:    (2) R < fastGLOBETROTTERv3.R parameter.infile.list samplefiles.infile.list recomratefiles.infile.list option --no-save > output.out
 
## example -- SOURCE/DATE/PROPORTION ESTIMATION:  R < fastGLOBETROTTERv3.R tutorial/paramfile.txt tutorial/samplefile.txt tutorial/recomfile.txt 1 --no-save > output.out
...
usage=function()
{
	print(noquote("run using: R < fastGLOBETROTTER.R parameter_infile painting_samples_filelist_infile recom_rate_filelist_infile --no-save > screen_output"))
	print(noquote("parameter input file format (NO defaults, all fields must be entered, even if not used):"))
	print(noquote("prop.ind: [0,1,0F,1F,0S,1S]"))
...
prop.ind=as.character(line.check(param.infile,0,"prop.ind:"))
if ((length(prop.ind)!=1)||(prop.ind!="0" && prop.ind!="1" && prop.ind!="1F" && prop.ind!="0F" && prop.ind!="1S" && prop.ind!="0S")) error.input.message(param.infile)
...
grid.range=as.character(line.check(param.infile,15,"curve.range:"))  ## integer values for where to start and end fitting on the admixture copying grid (put "F" at beginning of first value to turn off Pongsakorn's shifting of values)
shift.grid=1
if (sum(grep("F",grid.range[1]))>0)
{
	shift.grid=0
	grid.range[1]=strsplit(grid.range[1],split="F",fixed=T)[[1]][1]
}
grid.range=as.integer(grid.range)
...
if (prop.continue.ind==1 || prop.ind=="1F" || prop.ind=="0F" || prop.ind=="1S" || prop.ind=="0S") prop.save.addon = "_continue"

if (prop.continue.ind==1 && (prop.ind=="1" || prop.ind=="1F" || prop.ind=="1S"))
{
	print(paste("YOU HAVE SPECIFIED BOTH 'prop.ind' TO BE EQUAL TO 1 OR 1F AND 'prop.continue.ind' TO BE EQUAL TO 1 IN ",param.infile,". IF THERE IS AN EXISTING 'save.file.main', WHICH YOU WISH TO USE AS STARTING VALUES FOR YOUR ESTIMATION, THEN YOU WANT 'prop.continue.ind' TO BE 1. IF THERE IS NO EXISTING 'save.file.main', THEN YOU WANT 'prop.ind' TO BE 1. Exiting...",sep=''))
	q(save='no')
}
if ((prop.ind=="0" || prop.ind=="0F" || prop.ind=="0S") && max(c(prop.continue.ind,admix.curves.ind))==0)
{
...
################function that finds proportion contributed by make-up of each source, but FIXES SURROGATE POPS ALLOWED FOR EACH SOURCE
fitwhoadmixedFIXSURR=function(weights.mat,predmat,intercepts,fitdata,source1.surr,source2.surr){
	alpha.vec=seq(0.01,0.99,0.01)
	mu=0.5
	tol=1e-5
	results=vector(length=0)
	mu=sqrt(length(fitdata)*mean(fitdata^2))/(sqrt(length(fitdata)*mean(fitdata^2))+sqrt(length(intercepts)*mean(intercepts^2)))
	test.pop1=weights.mat%*%t(matrix(predmat[match(source1.surr,rownames(predmat)),],ncol=dim(predmat)[2]))
	colnames(test.pop1)=source1.surr
	test.pop2=weights.mat%*%t(matrix(predmat[match(source2.surr,rownames(predmat)),],ncol=dim(predmat)[2]))
	colnames(test.pop2)=source2.surr
	for(alpha in alpha.vec)
	{
		newpredmat=cbind(mu*sqrt(alpha)*sqrt(1-alpha)*test.pop1,-mu*sqrt(alpha)*sqrt(1-alpha)*test.pop2)
  		newpredmat=rbind(newpredmat,cbind((1-mu)*alpha*t(matrix(predmat[match(source1.surr,rownames(predmat)),],ncol=dim(predmat)[2])), (1-mu)*(1-alpha)*t(matrix(predmat[match(source2.surr,rownames(predmat)),],ncol=dim(predmat)[2]))))
  		target=c(mu*intercepts,(1-mu)*fitdata)
	                              ## deal with PCA randomly swapping negative and positive ends:
  		targetII=c(-1.0*mu*intercepts,(1-mu)*fitdata)
		res.target=nnls(newpredmat,target)$res
		res.targetII=nnls(newpredmat,targetII)$res
		ourguess=nnls(newpredmat,target)$x
		if (sum(res.targetII^2)<sum(res.target^2))
		{
			ourguess=nnls(newpredmat,targetII)$x
			target=targetII
		}
		                      ##########
  		orderi=order(ourguess[1:length(source1.surr)],decreasing=T)   ####use as a ranking
  		orderj=order(ourguess[(length(source1.surr)+1):length(ourguess)],decreasing=T)+length(orderi)
  		finish=0
  		for(i in orderi)   ## !!!! MUST BE EDITED TO WORK FOR ONE SOURCE FIXED WITH ONLY 1 SURROGATE !!!!!
		{
			for(j in orderj)
			{
				restrict=c(i,j)
				...
				##{
				##	restrict=restrict[-1]
				##	start.index.source2=2
				##}
				##if (length(orderj)==1) restrict=restrict[-2]
				temp=newpredmat[,-restrict]
				target2=target
				if (length(orderi)>1)
				{
					for(k in 1:(length(source1.surr)-1)) temp[,k]=temp[,k]-newpredmat[,restrict[1]]
					start.index.source2=length(source1.surr)
					target2=target2-newpredmat[,restrict[1]]
				}
				if (length(orderj)>1)
				{
					for(k in start.index.source2:dim(temp)[2]) temp[,k]=temp[,k]-newpredmat[,restrict[length(restrict)]]
  					target2=target2-newpredmat[,restrict[length(restrict)]]
				}
  				q=nnls(temp,target2)
  				x=q$x
  				newx=1:ncol(newpredmat)
  				newx[!((1:ncol(newpredmat))%in%restrict)]=x
  				newx[restrict[1]]=1-sum(x[1:(length(source1.surr)-1)])
  				newx[restrict[2]]=1-sum(x[start.index.source2:length(x)])
  				q$x=newx
      				if(q$x[i]>0 & q$x[j]>0) finish=1;
      				if(finish==1) break
			}
  		...
  		names(q$x)=colnames(newpredmat)
		pop1=q$x[1:length(source1.surr)]
		pop2=q$x[(length(source1.surr)+1):length(ourguess)]
		mixture.props.1.full=mixture.props.2.full=rep(0,dim(predmat)[1])
		mixture.props.1.full[match(names(pop1),rownames(predmat))]=pop1
		mixture.props.2.full[match(names(pop2),rownames(predmat))]=pop2
		mixturefit=alpha*mixture.props.1.full+(1-alpha)*mixture.props.2.full
		mixturefit=mixturefit[mixturefit>0]
	                    ## deal with PCA randomly swapping negative and positive ends:
		test.pop1.final=test.pop1%*%pop1
		test.pop2.final=test.pop2%*%pop2
		errors1a=intercepts-sqrt(alpha)*sqrt(1-alpha)*(test.pop1.final-test.pop2.final)
		errors1b=intercepts-sqrt(alpha)*sqrt(1-alpha)*(test.pop2.final-test.pop1.final)
		errors1=errors1a
		if (sum(errors1b^2)<sum(errors1a^2)) errors1=errors1b
		                      #######
###		errors2=fitdata-(alpha*copyvec.pop1+(1-alpha)*copyvec.pop2)
		munew1=mean(q$res[1:length(intercepts)]^2)/mu^2
		munew2=mean(q$res[(length(intercepts)+1):length(q$res)]^2)/(1-mu)^2
		lhood=-nrow(weights.mat)*log(munew1)/2-nrow(predmat)*log(munew2)/2
		mufitted=mu
		...
		pop2fit=pop2[pop2>0]
		names(pop2fit)=names(pop2)[pop2>0]
		curresults=list(alpha=alpha,pop1fit=pop1fit,pop2fit=pop2fit,mixturefit=mixturefit,lhood=lhood,curverelmse=curverelmse,proprelmse=proprelmse,avrelmse=avrelmse,curvecor=curvecor,propcor=propcor,avcor=avcor,resids=resids,mufitted=mufitted)
	 	results=c(results,curresults)
         }
	 return(results)
}

########function that finds proportion contributed by each source, but FIXES SOURCE MAKE-UP
fitonlyprops=function(weights.mat,predmat,intercepts,fitdata,mixture.props.1,mixture.props.2){
	alpha.vec=seq(0.01,0.99,0.01)
	mu=0.5
	tol=1e-5
	results=vector(length=0)
	mixture.props.1.full=mixture.props.2.full=rep(0,dim(predmat)[1])
	mixture.props.1.full[match(names(mixture.props.1),rownames(predmat))]=mixture.props.1
	mixture.props.2.full[match(names(mixture.props.2),rownames(predmat))]=mixture.props.2
	test=weights.mat%*%t(predmat)
	test.pop1=test%*%mixture.props.1.full
	test.pop2=test%*%mixture.props.2.full
	copyvec.pop1=copyvec.pop2=rep(0,dim(predmat)[2])
	for (i in 1:length(mixture.props.1)) copyvec.pop1=copyvec.pop1+mixture.props.1[i]*predmat[rownames(predmat)==names(mixture.props.1)[i],]
	for (i in 1:length(mixture.props.2)) copyvec.pop2=copyvec.pop2+mixture.props.2[i]*predmat[rownames(predmat)==names(mixture.props.2)[i],]
	mu=sqrt(length(fitdata)*mean(fitdata^2))/(sqrt(length(fitdata)*mean(fitdata^2))+sqrt(length(intercepts)*mean(intercepts^2)))
	for(alpha in alpha.vec)
	{
		errors1a=intercepts-sqrt(alpha)*sqrt(1-alpha)*(test.pop1-test.pop2)
		errors1b=intercepts-sqrt(alpha)*sqrt(1-alpha)*(test.pop2-test.pop1)
		errors1=errors1a
		if (sum(errors1b^2)<sum(errors1a^2)) errors1=errors1b
		errors2=fitdata-(alpha*copyvec.pop1+(1-alpha)*copyvec.pop2)
		munew1=mean(errors1^2)
		munew2=mean(errors2^2)
		lhood=-nrow(weights.mat)*log(munew1)/2-nrow(predmat)*log(munew2)/2
		mufitted=mu
		resids=c(errors1,errors2)
		curverelmse=mean(errors1^2)/mean(intercepts^2)
		...
		propcor=cor(fittedprops,fitdata)
		avcor=(curvecor+propcor)/2
		mixturefit=alpha*mixture.props.1+(1-alpha)*mixture.props.2
		mixturefit=mixturefit[mixturefit>0]
		curresults=list(alpha=alpha,pop1fit=mixture.props.1,pop2fit=mixture.props.2,mixturefit=mixturefit,lhood=lhood,curverelmse=curverelmse,proprelmse=proprelmse,avrelmse=avrelmse,curvecor=curvecor,propcor=propcor,avcor=avcor,resids=resids,mufitted=mufitted)
	 	results=c(results,curresults)
         }
	 return(results)
}
...
if (prop.ind=="1" || prop.ind=="1F" || prop.ind=="1S")
{
                       ## GET INITIAL FIT:
        if (prop.ind=="1") ourmix=getoverallfit(predmat,raw.copyprops.all)$x
	if (prop.ind=="1F" || prop.ind=="1S")
	{
		props.read=read.table(paste(save.file.props,prop.save.post,sep=''),nrows=2,as.is=TRUE)
		...
		mixture.props.1.orig=mixture.props.1
		if (prop.ind=="1S" && length(mixture.props.1)<2){print(paste("You have chosen 'prop.ind=1S', so you must have >1 surrogate pop for each source entered in ",save.file.props,prop.save.post,". Exiting.",sep='')); q(save='no')}
     		if (sum(is.element(names(mixture.props.1),donor.pops.all2))!=length(mixture.props.1)){ print(paste("You have chosen 'prop.ind=1F' or 'prop.ind=1S', but surrogate pops entered in ",save.file.props,prop.save.post," are not included among 'surrogate.popnames' in ",param.infile,". Exiting.",sep='')); q(save='no')}
		prop.val.est=as.double(props.read[2,1])
		props.read=read.table(paste(save.file.props,prop.save.post,sep=''),skip=2,nrows=2,as.is=TRUE)
		...
		mixture.props.2.orig=mixture.props.2
		if (prop.ind=="1S" && length(mixture.props.2)<2){print(paste("You have chosen 'prop.ind=1S', so you must have >1 surrogate pop for each source entered in ",save.file.props,prop.save.post,". Exiting.",sep='')); q(save='no')}
      		if (sum(is.element(names(mixture.props.2),donor.pops.all2))!=length(mixture.props.2)){ print(paste("You have chosen 'prop.ind=1F' or 'prop.ind=1S', but surrogate pops entered in ",save.file.props,prop.save.post," are not included among 'surrogate.popnames' in ",param.infile,". Exiting.",sep='')); q(save='no')}
...
if (prop.ind=="0" || prop.ind=="0F" || prop.ind=="0S")
{
                       ## GET INITIAL FIT:
     ...
     names(mixture.props.1)=as.character(props.read[1,2:dim(props.read)[2]])
     mixture.props.1.orig=mixture.props.1
     if (prop.ind=="0S" && length(mixture.props.1)<2){print(paste("You have chosen 'prop.ind=0S', so you must have >1 surrogate pop for each source entered in ",save.file.props,prop.save.post,". Exiting.",sep='')); q(save='no')}
     if (sum(is.element(names(mixture.props.1),donor.pops.all2))!=length(mixture.props.1)){ print(paste("You have chosen 'prop.ind=0' or 'prop.ind=0F' or 'prop.ind=0S', but surrogate pops entered in ",save.file.props,prop.save.post," are not included among 'surrogate.popnames' in ",param.infile,". Exiting.",sep='')); q(save='no')}
     ...
     names(mixture.props.2)=as.character(props.read[1,2:dim(props.read)[2]])
     mixture.props.2.orig=mixture.props.2
     if (prop.ind=="0S" && length(mixture.props.2)<2){print(paste("You have chosen 'prop.ind=0S', so you must have >1 surrogate pop for each source entered in ",save.file.props,prop.save.post,". Exiting.",sep='')); q(save='no')}
     if (sum(is.element(names(mixture.props.2),donor.pops.all2))!=length(mixture.props.2)){ print(paste("You have chosen 'prop.ind=0' or 'prop.ind=0F' or 'prop.ind=0S', but surrogate pops entered in ",save.file.props,prop.save.post," are not included among 'surrogate.popnames' in ",param.infile,". Exiting.",sep='')); q(save='no')}
     ourmix.pop1=ourmix.pop2=rep(0,length(donor.pops.all2))
     ourmix.pop1[match(names(mixture.props.1),donor.pops.all2)]=mixture.props.1
     ourmix.pop2[match(names(mixture.props.2),donor.pops.all2)]=mixture.props.2
     ourmix=prop.val.est*ourmix.pop1+(1-prop.val.est)*ourmix.pop2
...
if ((prop.ind=="1" || prop.ind=="1F" || prop.ind=="1S" || prop.continue.ind==1) && num.iterations>0)
{
	...
	if (fit.quality.orig==0)
	{
		print(paste("ISSUE WITH FIT AFTER ",a," ITERATIONS OF SOURCE/DATE ESTIMATION (E.G. ONLY ONE -- OR NEARLY ONE(?) -- SURROGATE GROUP TAKEN IN OPTIMAL FIT.) Exiting early....",sep=''))
		write("### INFERRED SOURCES AND DATES ('best-guess' conclusion: Unknown",file=paste(save.file.props,prop.save.addon,prop.save.post,sep=''),ncolumns=1)
     		write("#######################################",file=paste(save.file.props,prop.save.addon,prop.save.post,sep=''),ncolumns=1,append=TRUE)
		write("### 1-DATE FIT EVIDENCE, DATE ESTIMATE, SINGLE BEST-FITTING DONORS",file=paste(save.file.props,prop.save.addon,prop.save.post,sep=''),ncolumns=1,append=TRUE)
     		write(c("gen.1date","proportion.source1","maxR2fit.1date","fit.quality.1event","fit.quality.2events","bestmatch.event1.source1","bestmatch.event1.source2","proportion.event2.source1","bestmatch.event2.source1","bestmatch.event2.source2"),file=paste(save.file.props,prop.save.addon,prop.save.post,sep=''),ncolumns=10,append=TRUE)
		...
		write.table(rbind(NA,NA,NA,NA),file=paste(save.file.props,prop.save.addon,prop.save.post,sep=''),row.names=FALSE,col.names=FALSE,quote=FALSE,append=TRUE)
         ...
         ##############who admixed?
  	if (prop.ind=="1") results=fitwhoadmixed(weights.mat,predmat,intercepts.pca,raw.copyprops.all)
	if (prop.ind=="1F") results=fitonlyprops(weights.mat,predmat,intercepts.pca,raw.copyprops.all,mixture.props.1,mixture.props.2)
	if (prop.ind=="1S") results=fitwhoadmixedFIXSURR(weights.mat,predmat,intercepts.pca,raw.copyprops.all,names(mixture.props.1.orig),names(mixture.props.2.orig))
	results.fit=results
...
#            fit.quality2=get.fitted.results.1date.PC2[1]
	    ...
	    {
		if (prop.ind=="1") results.1date.PC2=fitwhoadmixed(weights.mat,predmat,pca.frac.val*intercepts.pca.1date.PC2,raw.copyprops.all)
		if (prop.ind=="1F") results.1date.PC2=fitonlyprops(weights.mat,predmat,pca.frac.val*intercepts.pca.1date.PC2,raw.copyprops.all,mixture.props.1,mixture.props.2)
		if (prop.ind=="1S") results.1date.PC2=fitwhoadmixedFIXSURR(weights.mat,predmat,pca.frac.val*intercepts.pca.1date.PC2,raw.copyprops.all,names(mixture.props.1.orig),names(mixture.props.2.orig))
	    ...
	    if (fit.quality.2date.PC1>0)
	    {
		if (prop.ind=="1") results.2date.PC1=fitwhoadmixed(weights.mat,predmat,pca.frac.val*intercepts.pca.2date.PC1,raw.copyprops.all)
		if (prop.ind=="1F") results.2date.PC1=fitonlyprops(weights.mat,predmat,pca.frac.val*intercepts.pca.2date.PC1,raw.copyprops.all,mixture.props.1,mixture.props.2)
 		if (prop.ind=="1S") results.2date.PC1=fitwhoadmixedFIXSURR(weights.mat,predmat,pca.frac.val*intercepts.pca.2date.PC1,raw.copyprops.all,names(mixture.props.1.orig),names(mixture.props.2.orig))
	    ...
	    if (fit.quality.2date.PC2>0)
	    {
		if (prop.ind=="1") results.2date.PC2=fitwhoadmixed(weights.mat,predmat,pca.frac.val*intercepts.pca.2date.PC2,raw.copyprops.all)
		if (prop.ind=="1F") results.2date.PC2=fitonlyprops(weights.mat,predmat,pca.frac.val*intercepts.pca.2date.PC2,raw.copyprops.all,mixture.props.1,mixture.props.2)
		if (prop.ind=="1S") results.2date.PC2=fitwhoadmixedFIXSURR(weights.mat,predmat,pca.frac.val*intercepts.pca.2date.PC2,raw.copyprops.all,names(mixture.props.1.orig),names(mixture.props.2.orig))
